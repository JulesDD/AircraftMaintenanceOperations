...
public record GetWorkOrderByIdQueryHandler(IAircraftMaintenanceDbContext DbContext, ICurrentUserService CurrentUser) : IQueryHandler<GetWorkOrderQueryByIdQuery, GetWorkOrderByIdQueryResult>
{
    ...
    {
        var domainUserId = CurrentUser.DomainUserId;

        var order = await DbContext.WorkOrders
            .Where(wo => wo.Id == query.WorkOrderId)
            .Where(wo => CurrentUser.Roles.Contains("Admin") || CurrentUser.Roles.Contains("MaintenanceSupervisor") || wo.AssignedTechnicianId == CurrentUser.DomainUserId)
        ...
        if (!isSupervisor && order.AssignedTechnicianId != CurrentUser.DomainUserId) throw new ForbiddenException("You are not authorized to access this work order!");
